...
(function () {
  ...
  function initPreloader() {
    ...
    // Fallback so the loader never sticks
  ...
  /* ------------------------------------------------------------------
     TYPING ANIMATION — realistic, human-like
     ------------------------------------------------------------------ */
  function initTyping() {
    ...
    function charDelay() {
      ...
      if (i > 0) {
        ...
        if (prev === '.' || prev === ',') return 340 + Math.random() * 380;
      }
    ...
    function typeTick() {
      if (i >= fullText.length) {
        setTimeout(function () { revealLine.classList.add('show'); }, 450);
        return;
      }
      i++;
      typedText.textContent = fullText.slice(0, i);
      // Small chance of a realistic hesitation (delete + retype)
      if (Math.random() < 0.045 && i > 4 && i < fullText.length - 2) {
        setTimeout(function () {
          typedText.textContent = fullText.slice(0, i - 2);
  ...
  function initReveal() {
    ...
    els.forEach(function (el) {
      ...
      if (!isNaN(delay) && delay > 0) el.style.transitionDelay = delay * 0.12 + 's';
    });
    ...
    var items = SKILLS.map(function (skill) {
      ...
      el.innerHTML =
        '<div class="skill-head">' +
          '<span class="skill-name"><span class="skill-dot"></span>' + escapeHtml(skill.name) + '</span>' +
          '<span class="skill-level">' + skill.level + '% · ' + label + '</span>' +
        '</div>' +
  ...
  /* ------------------------------------------------------------------
     PROJECTS — grid + detail modal (sharp, never-stretched images)
     ------------------------------------------------------------------ */
  function initProjects() {
    ...
    if (typeof PROJECTS === 'undefined' || !PROJECTS.length) {
      grid.innerHTML =
        '<p class="projects-note" style="grid-column:1/-1;color:var(--text-muted)">' +
        'No projects found. Open <code>js/projects.js</code> and add your projects.</p>';
      return;
    }
    ...
    PROJECTS.forEach(function (project, idx) {
      ...
      card.setAttribute('data-delay', (idx % 3));
      ...
      card.innerHTML =
        '<div class="project-media">' +
          ...
          '<img class="project-img" loading="lazy" alt="' + escapeAttr(project.title) + '" />' +
        '</div>' +
      ...
      var skeleton = $('.project-img-skeleton', card);

      function setupImage(src) {
        img.onload = function () { if (skeleton) skeleton.remove(); };
        img.onerror = function () {
          if (skeleton) skeleton.remove();
          showImageFallback(card, project.title);
        };
        img.src = src || '';
      }
      ...
      function openDetail() { openProjectModal(project); }
    ...
    initReveal(); // re-scan for the dynamically added cards
  }

  // Gradient placeholder used when an image is missing or fails to load
  function showImageFallback(card, title) {
    var media = $('.project-media', card);
    if (!media) return;
    var fallback = document.createElement('div');
    fallback.className = 'project-img-fallback';
    fallback.style.background = PLACEHOLDER_GRADS[Math.abs(hashStr(title)) % PLACEHOLDER_GRADS.length];
  ...
  function initModal() {
    ...
    $('#modalCloseAlt').addEventListener('click', closeProjectModal);
    ...
    document.addEventListener('keydown', function (e) {
      if (e.key === 'Escape' && modalEl.classList.contains('active')) closeProjectModal();
    });
  }
  ...
  function openProjectModal(project) {
    ...
    var skeleton = $('.modal-img-skeleton', media);
    var fallback = $('#modalImageFallback', modalEl) || null;
    ...
    if (project.image) {
      img.onload = function () {
        // NEVER upscale: cap display size at the image's real width so it
        // stays perfectly sharp (no pixelation / stretching).
        var containerW = media.clientWidth;
        var cap = Math.min(img.naturalWidth || containerW, containerW);
      ...
      img.onerror = function () {
        if (skeleton) skeleton.style.display = 'none';
        showModalImageFallback(project.title);
      };
  ...
  function showModalImageFallback(title) {
    if (!modalEl) return;
    var media = $('#modalMedia');
    var existing = $('.modal-img-fallback', media);
  ...
  /* ------------------------------------------------------------------
     CUSTOM CURSOR — desktop only, never on touch screens
     ------------------------------------------------------------------ */
  ...
  /* ------------------------------------------------------------------
     MARQUEE — infinite tech ticker (content duplicated in JS)
     ------------------------------------------------------------------ */
  function initMarquee() {
    var track = $('.marquee-track');
    if (!track) return;
    track.innerHTML += track.innerHTML; // two identical halves = seamless loop
  }
